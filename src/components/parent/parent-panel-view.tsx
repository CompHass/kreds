...
interface ParentPanelViewProps {
  ...
  familyChildren: Array<{
    // ATENÇÃO: não usar 'children' — conflito com prop reservada do React (Pitfall 2)
    id: string;
    displayName: string;
    accentColor: string;
    avatarPreset: string;
...
export function ParentPanelView({
  ...
  const formMode: "idle" | "create" | "edit" =
    editingId === "new" ? "create" : editingId !== null ? "edit" : "idle";

  // Tarefas filtradas por filho selecionado
  const filteredTasks =
    filter === "all" ? tasks : tasks.filter((t) => t.assigned.includes(filter));
          ...
          >
            ...
            {filteredTasks.length === 0 ? (
              ...
              >
                ...
                >
                  Clique em + Nova tarefa para criar a primeira tarefa desta
                  família.
                </p>
              </div>
            ) : (
